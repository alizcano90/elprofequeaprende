...
function bindActions() {
  ...
  byId("btnGenerate").addEventListener("click", generateOptimizedSchedule);
  ...
  byId("btnAddGroupSubject")?.addEventListener("click", addGroupSubject);
...
async function loadSeed() {
  ...
  if (saved) {
    EPQA.data = normalizeImportedData(saved.data);
    EPQA.slots = Array.isArray(saved.slots) ? saved.slots : (EPQA.data.slots || []);
  ...
  const response = await fetch("/horarios/api/seed.php");
  const payload = await response.json();
  EPQA.data = normalizeImportedData(payload.data);
  EPQA.slots = EPQA.data.slots.length ? EPQA.data.slots : buildEmptyProposal(EPQA.data);
  byId("jsonInput").value = JSON.stringify(payload.data, null, 2);
  fillFilters();
  renderMetrics();
  renderLoads();
  renderCatalogEditor();
...
function renderLoads() {
  ...
  byId("loadsTable").innerHTML = rows.map((load) => `
    <tr data-load-id="${escapeHtml(load.id)}">
      ...
      <td><input class="load-edit" data-field="hours" name="load-hours-${escapeHtml(load.id)}" type="number" min="1" value="${Number(load.hours || 0)}"></td>
      <td>
        <select class="load-edit" data-field="blockHours" name="load-block-${escapeHtml(load.id)}">
          <option value="1" ${blockHours(load) === 1 ? "selected" : ""}>No</option>
          <option value="2" ${blockHours(load) === 2 ? "selected" : ""}>2h</option>
          <option value="3" ${blockHours(load) === 3 ? "selected" : ""}>3h</option>
        </select>
      </td>
      <td>
        <select class="load-edit" data-field="rulePriority" name="load-priority-${escapeHtml(load.id)}">
          <option value="P0" ${loadRulePriority(load) === "P0" ? "selected" : ""}>P0</option>
          <option value="P1" ${loadRulePriority(load) === "P1" ? "selected" : ""}>P1</option>
          <option value="P2" ${loadRulePriority(load) === "P2" ? "selected" : ""}>P2</option>
        </select>
      </td>
      <td>${escapeHtml(load.room || load.roomId || "")}</td>
      <td><button class="ghost delete-load" type="button" data-load-id="${escapeHtml(load.id)}">Quitar</button></td>
    </tr>
  `).join("") || `<tr><td colspan="8" class="teacher-empty">No hay cargas con esos filtros.</td></tr>`;
  ...
  document.querySelectorAll(".load-edit").forEach((input) => {
    input.addEventListener("change", () => {
      const row = input.closest("tr");
      const load = findLoad(row.dataset.loadId);
      if (!load) return;
      if (input.dataset.field === "rulePriority") {
        load.rulePriority = normalizeRulePriority(input.value);
      } else {
        const value = Number(input.value);
        load[input.dataset.field] = Math.max(1, value);
      }
...
function renderCatalogEditor() {
  ...
  fillSelect("loadGroup", groupOptions(), "id", "name");
  fillSelect("loadSubject", subjectOptions(), "id", "name");
  fillSelect("loadRoom", roomOptions(), "id", "name");
...
function fillSelect(id, rows, valueKey, labelKey) {
  ...
  if (!select) return;
  const current = select.value;
  select.innerHTML = rows.map((row) => `<option value="${escapeHtml(row[valueKey])}">${escapeHtml(row[labelKey])}</option>`).join("");
...
function groupOptions() {
  if (Array.isArray(EPQA.data.groups)) {
    return EPQA.data.groups.map((group) => ({ id: group.id || group.name, name: group.name || group.id }));
  }
  return [...(EPQA.data.groups?.primary || []), ...(EPQA.data.groups?.secondary || [])].map((group) => ({ id: group, name: group }));
}
...
function roomOptions() {
  return (EPQA.data.rooms || []).map((room) => ({ id: room.id || room.name, name: room.name || room.id }));
}
...
function addLoad() {
  ...
  const room = roomOptions().find((item) => item.id === byId("loadRoom").value) || {};
  const load = {
    id: `load-${Date.now()}`,
    teacher: byId("loadTeacher").value,
    group,
    subject: byId("loadSubject").value,
    room: room.name || byId("loadRoom").value,
    roomId: room.id || byId("loadRoom").value,
    ...
    rulePriority: normalizeRulePriority(byId("loadRulePriority")?.value || "P0"),
    lockedTeacher: true
  };
...
function renderAvailabilityModal(teacherId = null) {
  ...
  const rows = days.map((day) => {
    ...
    const rowSiteValue = record.site || "";
  ...
  grid.innerHTML = rows;
  baseSite.value = days.map((day) => teacherAvailability[day]?.site).find(Boolean) || "";
}
...
function pendingLoadUnits() {
  const used = new Map();
  (EPQA.slots || []).forEach((slot) => used.set(slot.loadId, (used.get(slot.loadId) || 0) + slotDuration(slot)));
...
function renderPendingCard(load) {
  ...
  return `
    <div class="class-card pending-card ${duration > 1 ? "block-card" : ""}" data-load-id="${escapeHtml(load.id)}" data-duration="${duration}" style="background:${color};border-left-color:${borderColor(color)}">
      ...
      <small>Pendiente ${load.pendingIndex} · ${escapeHtml(load.room || load.roomId || "")}</small>
    </div>
  `;
}
  ...
  document.querySelectorAll(".slot-cell:not(.covered-cell)").forEach((cell) => {
    Sortable.create(cell, {
      ...
      onMove(event) {
        ...
        const slot = load
          ? slotFromLoad(load, event.to.dataset.day, Number(event.to.dataset.period))
      ...
      onEnd(event) {
        ...
        const targetPeriod = Number(event.to.dataset.period);
        if (event.item.dataset.loadId && event.to.classList.contains("slot-cell")) {
          ...
          const context = validatePlacementContext(load, event.to, true);
          if (!context.ok) {
            renderAvailableTray();
            renderBoard();
            cleanupDragState(event.item);
        ...
        const context = validatePlacementContext(slot, event.to, true);
        if (!context.ok) {
          renderBoard();
          cleanupDragState(event.item);
          return;
        }
        const swapResult = tryDirectSwap(slot, source, event.to);
        if (swapResult.done) {
          if (swapResult.ok) {
            confirmAction(
            ...
            confirmAction(
              ...
              () => {
                applyReplacementMove(slot.id, source, event.to.dataset.day, Number(event.to.dataset.period), context);
              },
              "Continuar"
            );
          } else {
...
function contextForSlotAt(slot, target) {
  ...
  const fixedSite = teacherFixedSite(slot.teacher);
  const targetSite = target.site || slot.site || slot.siteId || "";
  if (fixedSite && targetSite && !sameSite(fixedSite, targetSite)) {
    return { ok: false, message: `${slot.teacher} tiene sede fija ${fixedSite}; no puede quedar en ${targetSite}.` };
  }
...
function validatePlacementContext(item, targetCell, showAlert) {
  ...
  const fixedSite = teacherFixedSite(item.teacher);
  if (fixedSite && result.site && !sameSite(fixedSite, result.site)) {
    return placementDenied(`No se puede asignar: ${item.teacher} tiene sede fija ${fixedSite} y esta celda corresponde a ${result.site}.`, showAlert);
  }
...
function localP0Conflicts(slots, movingId) {
  ...
  const fixedSite = teacherFixedSite(moving.teacher);
  if (fixedSite && itemSite(moving) && !sameSite(fixedSite, itemSite(moving))) conflicts.push(`Sede fija docente: ${fixedSite}`);
  return conflicts;
}
...
function conflictDetailsForSlot(slot) {
  ...
  if (fixedSite && itemSite(slot) && !sameSite(fixedSite, itemSite(slot))) {
    details.push(`${slot.teacher} tiene sede fija ${fixedSite} y la clase quedaria en ${itemSite(slot)}`);
  }
...
async function auditNow() {
  const response = await fetch("/horarios/api/audit.php", {
    ...
    body: JSON.stringify({ slots: EPQA.slots, loads: EPQA.data.loads, rules: EPQA.data.rules })
  });
...
async function generateOptimizedSchedule() {
  ...
  showGenerationModal("Preparando cargas, restricciones y disponibilidad docente.", 4);
  await waitFrame();
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    const candidate = buildOptimizedProposal(attempt);
    if (!best || candidate.score < best.score) best = candidate;
    if (attempt % 12 === 0) {
      showGenerationModal(`Evaluando intento ${attempt}/${attempts}. Mejor faltante: ${best.unplacedHours}h, penalizacion ${Math.round(best.score)}.`, Math.round((attempt / attempts) * 92));
      await waitFrame();
    }
  ...
  const summary = missing
    ...
    : `Propuesta nueva generada con ${best.slots.reduce((sum, slot) => sum + slotDuration(slot), 0)} hora(s) ubicadas.`;
  notify(missing ? "Propuesta parcial" : "Propuesta optimizada", summary, missing ? "warning" : "success", Boolean(missing));
}

function buildOptimizedProposal(seed) {
  const days = shuffleArray(EPQA.data.days || ["Lunes", "Martes", "MiÃ©rcoles", "Jueves", "Viernes"], seed);
  const units = loadUnitsForOptimization(seed);
  const state = { slots: [], occupancy: new Set(), teacherDayHours: new Map(), unplaced: [], score: 0, hardConflicts: 0 };
...
function loadUnitsForOptimization(seed) {
  const units = [];
  (EPQA.data.loads || []).forEach((load) => {
    blockDurationsForLoad(load).forEach((duration, index) => {
...
function tryRelaxedPlacement(unit, days, state, seed) {
  ...
  for (let index = 0; index < unit.duration; index++) {
    ...
    placement.penalty += loadRulePriority(unit) === "P1" ? 450 : 160;
...
function compatibleRoomsForLoad(load) {
  ...
  const filtered = rooms.filter((room) => {
    ...
    const fixedSite = teacherFixedSite(load.teacher);
    if (fixedSite && room.site && !sameSite(fixedSite, room.site)) return false;
...
async function saveVersion(final) {
  const response = await fetch("/horarios/api/versions.php", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ final, counts: EPQA.audit.counts, snapshot: { slots: EPQA.slots, audit: EPQA.audit } })
  });
  const payload = await response.json();
  if (!payload.ok) {
